...
BOOL CEtherial::Deploy( )
{
	...
	return DefaultDeploy( "models/v_gauss.mdl", "models/p_gauss.mdl", ETHERIAL_DRAW, "etherial" );
}
...
void CEtherial::Holster( int skiplocal /* = 0 */ )
{
	...
	SendWeaponAnim( ETHERIAL_HOLSTER );
	m_fInAttack = 0;
}
...
void CEtherial::WeaponIdle( void )
{
	...
	else
	{
		...
		{
			iAnim = ETHERIAL_IDLE2;
			m_flTimeWeaponIdle = UTIL_WeaponTimeBase() + UTIL_SharedRandomFloat( m_pPlayer->random_seed, 10, 15 );
		}
		else
		{
			iAnim = ETHERIAL_FIDGET;
