...
static entvars_t    ev[ MAX_WEAPONS + 1 ]; // weapon limit break
...
static CBasePlayerWeapon *g_pWpns[ MAX_WEAPONS ]; // weapon limit break

float g_flApplyVel = 0.0;
int   g_irunninggausspred = 0;
int   g_irunningegonpred = 0;
int   g_irunningminigunpred = 0;
int   g_irunningcyclonepred = 0;
...
CM16 g_M16;
CREFLECTOR1 g_REFLECTOR1;
CEtherial g_Etherial;
CMinigun g_Minigun;
CPlasmarifle g_Plasmarifle;
CPlasmaGun g_PlasmaGun;
CCyclone g_Cyclone;
...
=====================
*/
BOOL CBasePlayerWeapon :: DefaultReload( int iClipSize, int iAnim, float fDelay, int body, float fDrop )
{
...
BOOL CBasePlayerWeapon :: DefaultDeploy( char *szViewModel, char *szWeaponModel, int iAnim, char *szAnimExt, int skiplocal, int	body )
{
	...
	g_irunningegonpred = false;
	g_irunningcyclonepred = false;	
...
void HUD_InitClientWeapons( void )
{
	...
	HUD_PrepEntity( &g_REFLECTOR1 , &player );
	HUD_PrepEntity( &g_Etherial , &player );
	HUD_PrepEntity( &g_Minigun	, &player );
	HUD_PrepEntity( &g_Plasmarifle  , &player );
	HUD_PrepEntity( &g_PlasmaGun    , &player );
	HUD_PrepEntity( &g_Cyclone	, &player );	
    ...
    HUD_PrepEntity( &g_Crossbowboom	, &player );
	HUD_PrepEntity( &g_Fists, &player );
	HUD_PrepEntity( &g_Fists2, &player );
...
void HUD_WeaponsPostThink( local_state_s *from, local_state_s *to, usercmd_t *cmd, double time, unsigned int random_seed )
{
	...
	switch ( from->client.m_iId )
	{
		...
		case WEAPON_REFLECTOR1:
			pWeapon = &g_REFLECTOR1;
			break;	
	
		case WEAPON_ETHERIAL:
			pWeapon = &g_Etherial;
			break;	
		...
		case WEAPON_CROSSBOWBOOM:
			pWeapon = &g_Crossbowboom;
			break;	
	...
	for ( i = 0; i < MAX_WEAPONS; i++ ) // weapon limit break
	{
		pCurrent = g_pWpns[ i ];
		if ( !pCurrent )
		{
...
//		pCurrent->m_flPumpTime			= pfrom->m_flPumpTime;
	...
	player.ammo_eagle		= (int)from->client.vuser2[1];
	...
	to->client.vuser2[1]				= player.ammo_eagle;
