...
//	DEFINE_FIELD( CHGrunt, m_flLastEnemySightTime, FIELD_TIME ), // don't save, go to zero
	...
	DEFINE_FIELD( CHGrunt, m_vecTossVelocity2, FIELD_VECTOR ),	
...
BOOL CHGrunt :: CheckRangeAttack2 ( float flDot, float flDist )
{
	...
	if (FBitSet( pev->weapons, HGRUNT_HANDGRENADE))
	{
		...
		Vector vecToss2 = VecCheckToss( pev, GetGunPosition(), vecTarget, 0.5 );
		...
		if ( vecToss2 != g_vecZero )
		{
			m_vecTossVelocity2 = vecToss2;
	...
	else
	{
		...
		Vector vecToss2 = VecCheckThrow( pev, GetGunPosition(), vecTarget, 1500, 0.5 );
...
void CHGrunt :: Shoot ( void )
{
	...
	switch ( RANDOM_LONG(0,2) )
	{
		case 0: EMIT_SOUND( ENT(pev), CHAN_WEAPON, "weapons/ak47_1.wav", 1, ATTN_NORM ); break;
		case 1: EMIT_SOUND( ENT(pev), CHAN_WEAPON, "weapons/ak47_2.wav", 1, ATTN_NORM ); break;
		case 2: EMIT_SOUND( ENT(pev), CHAN_WEAPON, "weapons/ak47_3.wav", 1, ATTN_NORM ); break;	
	}
	...
	MESSAGE_BEGIN( MSG_PVS, SVC_TEMPENTITY, vecShootOrigin );
		...
		WRITE_COORD(vecShootOrigin.x);	// X
		WRITE_COORD(vecShootOrigin.y);	// Y
		WRITE_COORD(vecShootOrigin.z);	// Z
...
void CHGrunt :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	...
	switch( pEvent->event )
	{
		...
		{
			EMIT_SOUND(ENT(pev), CHAN_WEAPON, "weapons/ak47_glauncher.wav", 0.8, ATTN_NORM);
			CGrenadeAK47::ShootContact( pev, GetGunPosition(), m_vecTossVelocity2 );
			
#ifndef CLIENT_DLL
	Vector vecSmokeOrigin;

	vecSmokeOrigin = GetGunPosition();	
...
void CHGrunt :: Precache()
{
	...
	PRECACHE_SOUND( "weapons/ak47_1.wav" );
	PRECACHE_SOUND( "weapons/ak47_2.wav" );
	PRECACHE_SOUND( "weapons/ak47_3.wav" );

	PRECACHE_SOUND( "weapons/ak47_glauncher.wav" );
	...
	case TASK_GRUNT_FACE_TOSS_DIR:
		{
			...
			MakeIdealYaw( pev->origin + m_vecTossVelocity2 * 64 );
