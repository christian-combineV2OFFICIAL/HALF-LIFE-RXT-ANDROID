...
void CPlasma :: IgniteThink( void  )
{
	...
	CBaseEntity::Create( "effect_plasma2", pev->origin, pev->angles, edict() );
...
void CPlasma :: Precache( void )
{
	...
	m_iGlow = PRECACHE_MODEL("sprites/particles_plasmared.spr");
	m_iExplode = PRECACHE_MODEL ("sprites/plasmared2.spr");
	m_iTrail = PRECACHE_MODEL("sprites/plasmatrail_red.spr");	
...
{
	m_pSprite = CSprite::SpriteCreate( "sprites/particles_plasmared.spr", pev->origin, FALSE );
	m_pSprite->SetAttachment( edict(), 0 );
	m_pSprite->pev->scale = 1;
	m_pSprite->pev->frame = 8;
	m_pSprite->pev->rendermode = kRenderTransAdd;
	m_pSprite->pev->renderamt = 255;	
	m_pSprite->SetTransparency( kRenderTransAdd, 255, 255, 255, 100, kRenderFxDistort );
}
//=========================================================
...
void CPlasma::Explode( void )
{
    ...
    CBaseEntity::Create( "effect_plasma", pev->origin, pev->angles, edict() );
	...
	::RadiusDamage ( pev->origin, pev, pevOwner, pev->dmg = 100, 400, CLASS_PLAYER_BIOWEAPON, DMG_GENERIC | DMG_ENERGYBEAM | DMG_RADIATION );
