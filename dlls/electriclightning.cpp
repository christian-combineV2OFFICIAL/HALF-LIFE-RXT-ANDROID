...
#define     VOLTIGORE_MAX_BEAMS		15
#define     VOLTIGORE_ZAP_RED 244
#define     VOLTIGORE_ZAP_GREEN 254
#define     VOLTIGORE_ZAP_BLUE 254
#define     VOLTIGORE_ZAP_BEAM "sprites/lgtning.spr"
#define     VOLTIGORE_ZAP_NOISE 80
#define     VOLTIGORE_ZAP_WIDTH 200
...
{	
	...
	while( iDrawn < iBeams && iTimes < ( iBeams * 3 ) )
	{
		...
		StreakSplash2( tr.vecEndPos, tr.vecPlaneNormal, 10, 200, 200, 2000 );
		StreakSplash2( vecOrigin, tr.vecEndPos, 6, 20, 50, 400 );
	    UTIL_ScreenShake( pev->origin, 500.0, 500, 5, 2000.0 );
        UTIL_ScreenFadeAll( Vector(0,0,255), 8, 0.5, 200, FFADE_IN );

        EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "weapons/lightninghit.wav", 1, ATTN_NORM, 10, 250);
		...
		{				
			...
			MESSAGE_BEGIN( MSG_BROADCAST, SVC_TEMPENTITY );
				...
				WRITE_COORD(pev->origin.x);
				WRITE_COORD(pev->origin.y);
				WRITE_COORD(pev->origin.z + 500); // reach damage radius over .3 seconds
				WRITE_SHORT(m_LightningTexture1 );
			...
			MESSAGE_BEGIN( MSG_BROADCAST, SVC_TEMPENTITY );
				...
				WRITE_COORD(pev->origin.y);
				WRITE_COORD(pev->origin.z + 500); // reach damage radius over .3 seconds
				WRITE_SHORT(m_LightningTexture2 );
				WRITE_BYTE( 0 ); // startframe
				WRITE_BYTE( 0 ); // framerate
				WRITE_BYTE( 8 ); // life
				WRITE_BYTE( 30 );  // width
...
{	
	...
	while( iDrawn < iBeams && iTimes < ( iBeams * 3 ) )
	{
		...
		StreakSplash2( tr.vecEndPos, tr.vecPlaneNormal, 10, 200, 200, 2000 );
		StreakSplash2( vecOrigin, tr.vecEndPos, 6, 20, 50, 400 );
	    UTIL_ScreenShake( pev->origin, 500.0, 1000.0, 1, 1000.0 );

        EMIT_SOUND_DYN(ENT(1000), CHAN_VOICE, "weapons/lightninghit.wav", 1, ATTN_NORM, 10, 250);
...
{	
	...
	while( iDrawn < iBeams && iTimes < ( iBeams * 3 ) )
	{
		...
		StreakSplash2( tr.vecEndPos, tr.vecPlaneNormal, 10, 200, 200, 2000 );
		StreakSplash2( vecOrigin, tr.vecEndPos, 6, 20, 50, 400 );
	    UTIL_ScreenShake( pev->origin, 200.0, 500.0, 1, 1000.0 );

	    CElectricDamage::ShootTimed( pev, tr.vecEndPos + pev->view_ofs + gpGlobals->v_forward * -20, gpGlobals->v_forward * 0, 0 );
