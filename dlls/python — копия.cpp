...
void CLaserFire2 :: Spawn( void )
{
	...
	pev->dmg = gSkillData.plrDmgSuperLaser;
}
...
void CLaserFire2 :: RocketTouch ( CBaseEntity *pOther )
{
	...
	SetThink ( SUB_Remove );
	LaserPistolTouch( pOther );
	pev->nextthink = gpGlobals->time + 1;// stick around long enough for the sound to finish!
}

void CLaserFire2::LaserPistolTouch2( TraceResult *pTrace, int bitsDamageType )
{
	...
	if ( !( pev->spawnflags & SF_ENVEXPLOSION_NOSPARKS ) )
	{
		...
		{
			Create( "spark_shower", pev->origin + gpGlobals->v_forward * -1, pTrace->vecPlaneNormal, NULL );
		}
	}			
...
void CLaserFire2::LaserPistolTouch( CBaseEntity *pOther )
{
	TraceResult tr;
	...
	LaserPistolTouch2( &tr, DMG_BLAST );
}
...
void CPython::PrimaryAttack()
{
	    ...
	    PLAYBACK_EVENT_FULL( FEV_NOTHOST, m_pPlayer->edict(), m_usFirePython, 0.0, (float *)&g_vecZero, (float *)&g_vecZero, 0, 0, 0, 0.0, 0, 0.0 );
