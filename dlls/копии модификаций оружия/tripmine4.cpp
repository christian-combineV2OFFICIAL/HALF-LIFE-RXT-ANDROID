...
void CTripmineGrenade :: MakeBeam( void )
{
	...
	UTIL_TraceLine( pev->origin, m_vecEnd, ignore_monsters, ENT( pev ), &tr );
	...
	m_pBeam = CBeam::BeamCreate( g_pModelNameLaser, 20 );
...
void StreakSplash3( const Vector &origin, const Vector &direction, int color, int count, int speed, int velocityRange )
{
...
void CTripmineGrenade :: BeamBreakThink( void  )
{
	...
	StreakSplash3( tr.vecEndPos, trace.vecPlaneNormal, 6, 20, 50, 400 );
	...
	{ 
       ClearMultiDamage( );
       CBaseEntity::Instance(tr.pHit)->TraceAttack(pev, 120, ( pev->origin - m_vecEnd), &tr, DMG_ENERGYBEAM | DMG_NEVERGIB );
